...
const MapControl = (props: React.PropsWithChildren<MapControlProps>) => {
  ...
  return (
    <div className={`${styles.countInfo}`} ref={ref}>{props.children}</div>
)
};

const MapControlSelection = (props: React.PropsWithChildren<MapControlProps>) => {
  const map = useGoogleMap()
  const ref = useRef(null)
  ...
  return (
    <div className={`${styles.searchContent}`} ref={ref}>{props.children}</div>
)
};
...
const Map = (props: IMapProps) => {
  ...
  const getMarkersPort = async () => {
    try {
      ...
      const responseAir = await axios.get(`http://localhost:5077/markers/ports/air`);
      const responseSea = await axios.get(`http://localhost:5077/markers/ports/sea`);
      setState(state);
  ...
  const onChangeSelectState = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    try {
      ...
      if (value) {
        ...
        const responsePorts = await axios.get(`http://localhost:5077/markers/ports`);
        const responseAir = await axios.get(`http://localhost:5077/markers/ports/air`);
      const responseSea = await axios.get(`http://localhost:5077/markers/ports/sea`);
...
<MapControlSelection position="RIGHT_TOP">
