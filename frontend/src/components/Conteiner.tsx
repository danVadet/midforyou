...
const Conteiner = (props: IContainerProps) => {
    ...
    const getContainer = async (id: number) => {
        ...
        try {
            if (id > 0) {
                const response = await axios.get(`http://localhost:5077/containers/capacity/${id}`);
                setContainer(response.data);
            }
                ...
                <div className={`${styles.calculadoraContent}`}>
                    ...
                    {<> <div className={`${styles.right}`}>
                        <div className={`${styles.info_container}`}>
                            ...
                            <img src={container.image ? `${container.image}` : `./assets/containerPlaceholder.png`} alt="" />
                            ...
                            {product.pesoTotal <= container.capacidadePeso && product.volumeTotal <= container.capacidadeVolume ?
                                ...
                                </> : showContainerMessage && <Message type='error' message='Esse container não cabe' />}
                        </div>
                    </div>
                    </>
                    }
