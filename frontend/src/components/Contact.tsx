...
const Contact = (props: IContactProps) => {
  ...
  const validate = (visitor: Visitor) => {
    ...
    if (!visitor.email) {
      errors.email = `${props.emailRequiredContact}`;
  ...
  return (
    ...
    <div className={`${styles.contactImage}`} >
     ...
     <svg viewBox="0 0 24 24" width="30" height="30" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          {sentData ? <button className={`${styles.sentButton}`}>{props.buttonSent}</button> : <button className={`${styles.sendButton}`} >{props.buttonSend}</button>}
        </form>
      </div>
    </div>
  );
