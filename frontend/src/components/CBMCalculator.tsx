...
export const CBMCalculator = () => {
    ...
    const onSubmit = async (e: FormEvent) => {
        ...
        if (errors && Object.keys(errors).length > 0) {
            ...
            setProduct({
                id: 0, name: "", length: 0, width: 0, height: 0, weight: 0, quantity: 0, volume: 0, volumeTotal: 0, weightTotal: 0, measureUnit: MeasureUnit.m, weightUnit: WeightUnit.kg });
            getProducts();
            getSumTotalWeight();
            getSumTotalVolume();
            getSumTotalQuantity();
    ...
    const getWeightUnits = async () => {
        const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/weightUnits`);
                    ...
                    <div>
                        <form onSubmit={(e) => onSubmit(e)} className={`${styles.formContainer}`}>
                            ...
                            <div className={`${styles.formGroup}`}>
                                ...
                                <input type="number" name="length" value={product.length || ""} className={product.length ? "" : `${errors.length && `${styles.invalid}`}`} placeholder={`Digite o comprimento (${product.measureUnit})`} onChange={(e) => onChange(e)} />
                                 {product.measureUnit}
                                {product.length ? "" : errors.length && <p className={styles.formError}>{`${errors.length}`}</p>}

                            </div>
                            <div className={`${styles.formGroup}`}>
                                <input type="number" name="width" value={product.width || ""} className={product.width ? "" : `${errors.width && `${styles.invalid}`}`} placeholder={`Digite a lagura (${product.measureUnit})`} onChange={(e) => onChange(e)} />
                                {product.width ? "" : errors.width && <p className={styles.formError}>{`${errors.width}`}</p>}
                            </div>

                            <div className={`${styles.formGroup}`}>
                                <input type="number" name="height" value={product.height || ""} className={product.height ? "" : `${errors.height && `${styles.invalid}`}`} placeholder={`Digite a altura (${product.measureUnit})`} onChange={(e) => onChange(e)} />
                            ...
                            <div className={`${styles.formGroup}`}>
                                <input type="number" name="weight" value={product.weight || ""} className={product.weight ? "" : `${errors.weight && `${styles.invalid}`}`} placeholder={`Digite o peso (${product.weightUnit})`} onChange={(e) => onChange(e)} />
                            ...
                            {products.length === 0 ? (<li>Produto não adicionado</li>) : (products.map((product, index) => (
                                <li key={index} className={`${styles.product_item}`}>
                                    ...
                                    {openEditModal && <EditProductModal
                                        closeModal={() => setOpenEditModal(false)}  getProducts={getProducts} getSumTotalWeight={getSumTotalWeight} getSumTotalVolume={getSumTotalVolume} getSumTotalQuantity={getSumTotalQuantity} currentProduct={currentProduct} measureUnits={measureUnits} getMeasureUnits={getMeasureUnits} weightUnits={weightUnits} getWeightUnits={getWeightUnits} getContainer={() => getContainer(selectedContainer.id)} selectedContainerId={selectedContainer.id} setShowEditMessage={showEditMessage} />}
                        ...
                        <div className={`${styles.selectContainer}`}>
                            ...
                            {!loading ? <div> <img className={`${styles.imageContainer}`} src={selectedContainer.image ? `${process.env.REACT_APP_API_BASE_URL}/${selectedContainer.image}` : `./assets/containerPlaceholder.png`} alt="" /> </div> :
                                ...
                                <div className={`${styles.containerCargoTotalWeight}`}>
                                        ...
                                        {sumTotalWeightLb ? <><ProgressBar value={pctWeightLb} />
                                        ...
                                        <div className={`${styles.containerCargoTotalVolume}`}>
                                            ...
                                            {sumTotalVolumeFt3 ? <> <ProgressBar value={pctVolumeFt3} />
                                            <div className={`${styles.labelCapacityVolume}`}>
